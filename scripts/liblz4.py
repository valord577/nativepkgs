...
def _source_download():
    ...
    if not os.path.exists(os.path.abspath(os.path.join(_env['SUBPROJ_SRC'], '.git'))):
        ...
        _env['FUNC_SHELL_DEVNUL'](cwd=_env['SUBPROJ_SRC'], args=[shutil.which('git'), 'remote', 'add', 'x', 'https://salsa.debian.org/debian/lz4.git'])
...
def _build_step_00():
    ...
    args = [
        _ctx['CMAKE_CMD'],
        '-S', f'{_env["SUBPROJ_SRC"]}/build/cmake',
        '-D',  'LZ4_BUILD_CLI:BOOL=0',
        '-D',  'LZ4_POSITION_INDEPENDENT_LIB:BOOL=1',
    ]
