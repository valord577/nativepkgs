...
def _build_step_1():
    x.run_as_subprocess(args=['cmake', '--build', ctx.args.pkg_buld_dir, '-j', f'{x.detect_cpu_count()}'])
def _build_step_2():
    x.run_as_subprocess(args=['cmake', '--install', ctx.args.pkg_buld_dir, '--strip'])
